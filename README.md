...
*         *********         *
***************************
As the orginal commands ```npm run build-db``` and ```npm run clean-db``` do NOT work on windows, i have changed the commands so that they work in Windows which is the OS that I am working on. To run these commands on Linux or Unix OS, you will need to run 
```npm run build``` and ```npm run clean``` instead. If you are running on windows OS, you may continue using ```npm run build-db``` and ```npm run clean-db```.


#### How to run the application ####

* Enter npm run start or npm run dev for auto refresh server on save file.
* Type http://localhost:3000/author/home into browser to access the page. 
* You may click on the navbar links to traverse between Article and Settings Page. 
